...
# store all spec3[-1] values
final_spec3 = []

for seed in range(1, iters):
    _, _, _, spec3 = parse_logfile('log{}.spparks'.format(seed))
    final_spec3.append(spec3[-1])

# calculate IQR for spec3[-1]
Q1 = np.percentile(final_spec3, 25)
Q3 = np.percentile(final_spec3, 75)
IQR = Q3 - Q1
...
for seed in range(1, iters):
    ...
    if outlier_range[0] <= spec3[-1] <= outlier_range[1]:  # if spec3[-1] is not an outlier
        spec1_avg += spec1
        spec2_avg += spec2
        spec3_avg += spec3
...
b, f = solution.x
append_to_file('diffrate.txt', str(b)+" "+str(f))
