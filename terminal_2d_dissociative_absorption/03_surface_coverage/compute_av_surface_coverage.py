...
for i in range(1, Nruns+1):
    coverage = np.loadtxt(os.path.join(resdir, "data{}.txt".format(i)), skiprows=501)[:, 1]
...
data_to_save = np.column_stack((np.loadtxt(os.path.join(resdir, "data1.txt"), skiprows=501)[:, 0], average_cov))
